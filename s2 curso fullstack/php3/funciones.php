...
function getPersona($email)
{
    global $datosEnArray;
    foreach ($datosEnArray as $persona) {
    ...
    foreach ($datosEnArray as $persona) {
        foreach ($persona as $datos => $valor) {
            if ($persona["email"] == $email && $persona["password"] !== $password) {
                $errores[] = "El password ingresado es incorrecto. Por favor intentalo nuevamente.";
             } else if ($persona["email"] !== $email) { 
                $errores[] = "El email ingresado no es reconocido.";
             }
        }
    }
    return $errores;
